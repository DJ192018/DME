...
if __name__ == "__main__":
    pathFolder = r"C:\Users\Dhanshree\Desktop\retinal-exudates-detection-master\ddb1_fundusimages"
    filesArray = [x for x in os.listdir(pathFolder) if os.path.isfile(os.path.join(pathFolder,x))]
    DestinationFolder = r"C:\Users\Dhanshree\Desktop\poster\MaculoPathy\DMEE"
    if not os.path.exists(DestinationFolder):
        os.mkdir(DestinationFolder)
    for file_name in filesArray:
        file_name_no_extension = os.path.splitext(file_name)[0]
        fundus = cv2.imread(pathFolder+'\\'+file_name)
        #image = cv2.imread(r"C:\Users\Dhanshree\Desktop\retinal-exudates-detection-master\ddb1_fundusimages\image005.png")
        fundus = cv2.resize(fundus,(800,615))
        fundus_mask = cv2.imread(r"C:\Users\Dhanshree\Desktop\retinal-exudates-detection-master\MASK.bmp")
...
#  Morphological filters on image
# for OD detection applying alternate sequential filtering (3 times closing opening)
        r1 = cv2.morphologyEx(contrast_enhanced_green_fundus, cv2.MORPH_OPEN, cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(5,5)), iterations = 1)
        R1 = cv2.morphologyEx(r1, cv2.MORPH_CLOSE, cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(5,5)), iterations = 1)
        r2 = cv2.morphologyEx(R1, cv2.MORPH_OPEN, cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(11,11)), iterations = 1)
        R2 = cv2.morphologyEx(r2, cv2.MORPH_CLOSE, cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(11,11)), iterations = 1)
...
# Masking OD                                             
        img = cv2.medianBlur(R3, 51)
        (minVal, maxVal, minLoc, maxLoc) = cv2.minMaxLoc(img)
        (x,y)=maxLoc
        #opaque circle
        v=cv2.circle(edge_candidates, maxLoc, 55 , (0, 0, 0), -1) #cv2.circle(img, center, radius, color[, thickness[, lineType[, shift]]])
...
# =============================================================================
# =============================================================================
        candidates_vector = np.reshape(edge_candidates,(edge_candidates.size,1))/255
        temp = 0
        counter = 0
        this_image_rows = 0
        qq=0
        with open('training.csv', 'a') as csvfile:
            ...
            while counter < feature1.shape[0]:
                if candidates_vector[counter,0] == 1:
                    ...
                    filewriter.writerow([feature2[counter,0],feature3[counter,0],feature4[counter,0],feature5[counter,0],feature6[counter,0],feature8[counter,0],average_intensity[counter,0],average_hue[counter,0],average_saturation[counter,0]])
                counter = counter + 1
...
im = Image.open(r"C:\Users\Dhanshree\Desktop\poster\diaretdb1_v_1_1\diaretdb1_v_1_1\resources\images\ddb1_fundusimages\image006.png") #relative path to file
